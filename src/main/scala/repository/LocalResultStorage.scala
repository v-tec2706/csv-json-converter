...
class LocalResultStorage(config: Config) extends ResultStorage {
  ...
  override def storageFile(taskId: TaskId): (ZSink[Any, Throwable, String, Byte, Long], Path) = {
    val path = s"${config.fileStoragePath}/$filePrefix-${taskId.id}.json"
    (ZSink.fromFileName(path).contramapChunks[String](_.flatMap(_.getBytes)), new Path(path))
  }
